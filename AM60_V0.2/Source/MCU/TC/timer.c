...
*/
volatile uint32_t millis = 0;           // seconds counter
/**
@brief          use to impliment a LCD on-off
*/
...
static void TIM_init_F0(void){
    TCF0.CTRLA = TC_CLKSEL_DIV1024_gc;          // Set prescaler to 1024
    TCF0.PER = __TF0_PER;                           /* Set period for 1 second
                                                   (based on 16 MHz clock)*/
    TCF0.CNT = 0;                               // Clear the counter
...
__interrupt void TF0_compA(void) {
    millis++; // Increment the seconds counter
}
